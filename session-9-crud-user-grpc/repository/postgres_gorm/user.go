...
func (r *userRepository) GetAllUsers(ctx context.Context, pageSize int, offset int) ([]entity.User, error) {
	var users []entity.User
	if err := r.db.WithContext(ctx).Select("id", "name", "email", "password", "created_at", "updated_at").Limit(pageSize).Offset(offset).Find(&users).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return users, nil
		}
