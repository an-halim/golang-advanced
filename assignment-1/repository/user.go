...
func (r *userRepository) GetUserByID(ctx context.Context, id int) (entity.User, error) {
	...
	if err := r.db.WithContext(ctx).Preload("Submissions").First(&user, id).Error; err != nil {
...
func (r *userRepository) GetAllUsers(ctx context.Context, pageSize int, offset int) ([]entity.User, error) {
	...
	if err := r.db.WithContext(ctx).Preload("Submissions").Limit(pageSize).Offset(offset).Find(&users).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return users, nil
		}
